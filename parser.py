...
class PartialParse(object):
    ...
    def complete(self):
        ...
        # Stack only has root
        has_only_root = len(self.stack) == 1 and self.stack[0] == 0

        # Buffer is empty
        is_buffer_empty = self.next == len(self.sentence)

        return has_only_root and is_buffer_empty
    ...
    def parse_step(self, transition_id, deprel=None):
        ...
        if transition_id == self.left_arc_id:
            # stack must have at least 2 items (including root)
            if len(self.stack) >= 2:
                head = self.stack[-1]
                dep = self.stack[-2]
                self.arcs.append((head, dep, deprel))
                self.stack.remove(dep)
            else:
                raise ValueError('stack must have at least 2 items for left arc operation')
            ...
            if len(self.stack) >= 2:
                ...
                raise ValueError('stack must have at least 2 items for right arc operation')
            ...
            raise ValueError('invalid transition id is given')
    ...
    def get_n_leftmost_deps(self, sentence_idx, n=None):
        ...
        deps = [dep for (head, dep, deprel) in self.arcs if head == sentence_idx]
    ...
    def get_n_rightmost_deps(self, sentence_idx, n=None):
        ...
        # else: (if n == None or len(deps < n)
        #      just return deps in reverse order
    ...
    def get_oracle(self, graph):
        ...
        *ALSO IMPORTANT* make sure to use the values `self.left_arc_id`,
        ...
        if len(self.stack) == 1:
            ...
            first = self.stack[-1] # first item on stack
            second = self.stack[-2] # second item on stack

            # check if left-arc is possible
            if graph.nodes[second]['head'] == first:
                transition_id = self.left_arc_id
            
                # obtain dependency relationship between head and dependent
                head_deps = graph.nodes[first]['deps'].items()
                deprel, dep = [(deprel, dep) for (deprel, dep) in head_deps if dep == [second]][0]


            # check if right-arc is possible
            # unlike left-arc, we need to check that first item has no
            # arc-right dependents in the buffer.
            elif graph.nodes[first]['head'] == second:

                arc_right_deps = get_right_deps(graph.nodes[first])

                if all(arc_right_dep < self.next for arc_right_dep in arc_right_deps):
                    transition_id = self.right_arc_id
                    
                    # obtain dependency relationship between head and dependent
                    head_deps = graph.nodes[second]['deps'].items()
                    deprel, dep = [(deprel, dep) for (deprel, dep) in head_deps if dep == [first]][0]

                else:
                    transition_id = self.shift_id

           # left-arc and right-arc cannot be done at this state, so it must be shift
            else:
                transition_id = self.shift_id
...
def minibatch_parse(sentences, model, batch_size):
    ...
    # initialize a list of partial_parses, one for each sentence
    partial_parses = []
    ...
    while len(unfinished_parses) > 0:
        ...
        for i in range(len(minibatch)):
            transition_id, deprel = td_pairs[i]
            partial_parse = minibatch[i]
            try:
                partial_parse.parse_step(transition_id, deprel)
            except ValueError:
                # remove a stuck partial_parse
                unfinished_parses.remove(partial_parse)

            # remove a completed parse
