...
class ParserModel(Model):
    ...
    def add_embeddings(self):
        ...
        ** Embedding matrices should be variables, not constants! **
        ...
        word_embedding_matrix = tf.Variable(self.word_embeddings)
        tag_embedding_matrix = tf.get_variable("tag_embeddings", shape=[self.config.n_tag_ids, self.config.embed_size], initializer=xavier_init)
        deprel_embedding_matrix = tf.get_variable("deprel_embeddings", shape=[self.config.n_deprel_ids, self.config.embed_size], initializer=xavier_init)

        embedded_word_ids = tf.nn.embedding_lookup(word_embedding_matrix, self.word_id_placeholder)
        embedded_tag_ids = tf.nn.embedding_lookup(tag_embedding_matrix, self.tag_id_placeholder)
        embedded_deprel_ids = tf.nn.embedding_lookup(deprel_embedding_matrix, self.deprel_id_placeholder)

        word_embeddings = tf.reshape(embedded_word_ids, [-1, self.config.n_word_features * self.config.embed_size])
        tag_embeddings = tf.reshape(embedded_tag_ids, [-1, self.config.n_tag_features * self.config.embed_size])
        deprel_embeddings = tf.reshape(embedded_deprel_ids, [-1, self.config.n_deprel_features * self.config.embed_size])
    ...
    def add_prediction_op(self):
        ...
        W_w = tf.get_variable("W_w", shape=[self.config.n_word_features * self.config.embed_size, self.config.hidden_size], initializer=xavier_init)
        W_t = tf.get_variable("W_t", shape=[self.config.n_tag_features * self.config.embed_size, self.config.hidden_size], initializer=xavier_init)
        W_d = tf.get_variable("W_d", shape=[self.config.n_deprel_features * self.config.embed_size, self.config.hidden_size], initializer=xavier_init)
        ...
        h = tf.nn.relu(tf.matmul(x_w, W_w) + tf.matmul(x_t, W_t) + tf.matmul(x_d, W_d) + b1)
    ...
    def add_loss_op(self, pred):
        ...
        """
        ### BEGIN YOUR CODE
        # find the loss for each set and average them
